...
//        bitmap.compress(Bitmap.CompressFormat.JPEG, 100, os);
//        int quality=100;
//	    while( os.toByteArray().length / 1024>200) {//判断如果图片大于1M,进行压缩避免在生成图片（BitmapFactory.decodeStream）时溢出    
//	        os.reset();//重置baos即清空baos  
//	        quality-=10;
//	        bitmap.compress(Bitmap.CompressFormat.JPEG, quality, os);//这里压缩50%，把压缩后的数据存放到baos中  
//	    }
...
//        return bitmap;
	    //2 压缩好比例大小后再进行质量压缩
//        return compressImage(bitmap, 200); // 这里再进行质量压缩的意义不大，反而耗资源，删除
        //3
        return bitmap;
	}
	...
	public Bitmap ratio(String srcPath) {
		BitmapFactory.Options newOpts = new BitmapFactory.Options();
...
//	    return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
	 ...
	 * @return
	 */
	public Bitmap scale(Bitmap image,int size){
